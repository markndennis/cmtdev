...
class Exammodel extends CI_Model {
    ...
    function exam_end($pin) {
		...
		$to = 'onlinejuris@cmtbc.ca,adam@veritagroup.com,info@cmtbc.ca';
		//$to = 'adam@veritagroup.com,marknden@gmail.com';
        $subject = 'Examinee ' . $pin . ' has completed their Bylaw Workshop Evaluation';
    ...
    function exam_status($pin) {
		// function tests for exam status
        $query = $this->db->get_where("Results", array('Pin' => $pin));
        $result = $query->row_array();
    ...
    function examresults($pin) {
		...
		if ($this->test_for_output_record($pin) != NULL){
			$this->db->where('Pin', $pin);
    ...
    function email_results($pin, $correct) {
        if (!$this->session->userdata('scoreemail')) {
			...
			$EMAILTO = 'marknden@gmail.com,adam@veritagroup.com';
